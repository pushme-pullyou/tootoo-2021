...
GRV.onLoadTree = function ( json ) {
	...
	window.addEventListener( "hashchange", GRV.onHashChange )
...
GRV.onHashChange = function() {
	...
	link = GRVdivGitHubRepoTreeView.querySelectorAll( "a")
	
	console.log( "", link );
	
	item = Array.from( link ).find( a => 
		a.getAttribute( "href").includes( location.hash.slice( 1 )) )
