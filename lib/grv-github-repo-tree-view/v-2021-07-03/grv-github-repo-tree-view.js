...
/* global COR, MNU, GRVdivDetails, GRVdivFolders, GRVdivGitHubRepoTreeView, divPopUp */
// jshint esversion: 6
...
GRV.menuDefault = "Display all folders and files";
...
<details id=GRVdet >
	...
	<p>Numerous folders and files for you to look at!</p>
	...
	<div class=divNavResize>
		...
		<p><button id=GRVbutMenu onclick=GRV.toggleMenu() >${ GRV.menuDefault }</button></p>
...
GRV.onLoadTree = function ( json ) {
	...
	if ( GRV.getFiles === GRV.getFilesCurated ) {
		...
		filesRoot = files
			...
			.map( ( item, i ) => `
		<div class=GRVdiv >
			<a href="${ COR.source }${ item }" title="Source code on GitHub. Edit me!" target="_blank" >
			${ COR.iconGitHub }</a>
...
GRV.subtreesToDetails = function ( subtrees, files ) {
	...
	const htmArr = subtrees.map( ( subtree, index ) => {
		...
		<details id=GRVdet${ index } class="GRVdet" onToggle = GRV.onDetToggle("${ subtreeTitle }") ${ detOpen }>
			<summary class="summary-secondary" >${ subtreeTitle }</summary>
			${ filesHtm.join( "" ) }
		`;
	} );
...
GRV.toggleMenu = function () {
	...
	if ( GRVbutMenu.innerHTML === GRV.menuDefault ) {
		...
		GRVbutMenu.innerHTML = GRV.menuDefault;
		GRV.getFiles = GRV.getFilesCurated;
	}
