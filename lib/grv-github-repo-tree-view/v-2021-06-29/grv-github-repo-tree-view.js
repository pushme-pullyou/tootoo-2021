...
GRV.init = function ( { user = COR.user, repo = COR.repo, branch = COR.branch } = {} ) {
	...
	GRV.iconOctoCat = `<img width=14 src="${ COR.path }lib/assets/icons/octicon.svg">`;
	GRV.link = `<img width=14 src="${ COR.path }lib/assets/icons/icon-link-external.svg">`;
...
GRV.getRepo = function (user = GRV.user, repo = GRV.repo, branch = GRV.branch  ) {
	...
	GRV.urlViewer = COR.repo.startsWith( COR.user ) ? `https://${ COR.user }.github.io/` : `https://${ COR.user }.github.io/${ repo }/`;
	GRV.links = undefined;
	//location.hash = "";
