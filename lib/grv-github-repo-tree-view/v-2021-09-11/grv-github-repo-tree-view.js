...
GRV.intro = `
<p>This menu enables you to display all folders and files in the GitHub repository in a tree view.</p>
`
GRV.urlViewer = "";
...
GRV.init = function ( { user = COR.user, repo = COR.repo, branch = COR.branch } = {} ) {
	...
	GRV.ignoreFolders = COR.ignoreFolders.slice();
...
<details id=GRVdet >
	...
	<summary id=GRVsumRepo class="summary-primary gmd-1" title="View selected items">
		<span id=GRVsumTitle >${ COR.repo } repository files</span>
...
GRV.onLoadTree = function ( json ) {
	...
	if ( GRV.getFiles === GRV.getFilesCurated ) {
		...
		filesRoot = files
			...
			.map( ( item, i ) => `
		<div class=GRVdiv >
			<a href="#${ COR.path}${ item }" >${ item.split( "." ).shift().replace( /-/g, " " ) }</a>
		</div>`);
		...
		<div class=GRVdiv >
			...
			${ COR.iconGitHub }</a>
			<a href="#${ COR.path}${ item }" >${ item.split( "/" ).pop() }</a>
...
GRV.onHashChange = function () {
	...
	const str = location.hash ? location.hash.slice( 1 ) : COR.path + COR.defaultFile;
...
GRV.getFilesAll = function ( subtree, files ) {
	...
	const filtered = files
		...
		<div class=GRVdiv>
			...
			${ COR.iconGitHub }</a>
			<a href="#${ COR.path}${ item }" title="">${ item.split( "/" ).pop() }</a>
...
GRV.getFilesCurated = function ( subtree, files ) {
	...
	const filtered = files
		...
		<div style="margin: 5px 0;" >
			<a href="#${ COR.path}${ item }" title="" onclick="JavaScript:if(window.innerWidth<640||window.innerHeight<500){navMenuDet.open=false;}" >${ item.split( "/" ).pop().split( "." ).shift().replace( /-/g, " " ) }</a>
		</div>`);
