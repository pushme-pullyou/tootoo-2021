	...
	const htm = `
			<details open>
				...
				<div class=divNavResize>
					...
					<label for="ice-cream-choice">Choose a user:</label>
					<input type=search list="GORlistUsers" id="GOPinpUser" oninput=GOR.getOrganization(this.value); placeholder="pushme-pullyou" name="Select a GitHub user or organization" />

					<datalist id="GORlistUsers">
						<option value="pushme-pullyou" >
						<option value="ladybug-tools" >
						<option value="theo-armour">
						<option value="mostaphaRoudsari">
						<option value="chrismackey">
					</datalist>
...
GOR.getOrganization = function ( user = GOR.user ) {
...
GOR.onLoadTree = function ( repos = GOR.repo ) {
...
<div>
	<select id=GORinpRepo oninput=GOR.getRepo(this); size=10 >
	${ texts.join( "" ) }
	</select>
</div>
...
GOR.getRepo = function ( that ) {
	...
	if ( json ) {
		...
		GOR.urlApi = `https://api.github.com/repos/${ GOR.user }/${ GOR.repo }/git/trees/${ json.default_branch }?recursive=1`;
...
<div>User: <a href=https://github.com/${ GOR.user} target="_blank">${ GOR.user }</a>
Repo: <a href="${ json.html_url }" target="_blank">${ json.name }</a></div>
