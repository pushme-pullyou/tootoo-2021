...
GOR.init = function ( { user = COR.user, repo = COR.repo, path = COR.path } = {} ) {
	GOR.user = user;
	GOR.repo = repo;
	GOR.path = path;
	...
	const htm = `
			<details id=GORdet >
				...
				<summary class="summary-primary gmd-1" title="View selected items">
					...
					${ MNU.addInfoBox( "View all the repositories belonging to a GitHub user or organization" ) }

				</summary>

				<div class=divNavResize>

					<label for="GORinpUser">Select a GitHub user or type one in:</label>
...
GOR.getRepo = function ( that ) {
	...
	if ( json ) {
		...
		// if ( window.GRVdivGitHubRepoTreeView ) {
