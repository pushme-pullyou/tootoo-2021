...
HBJ.processOrphanShades = function () {
	...
	HBJ.orphans = [ "orphaned_apertures", "orphaned_doors", "orphaned_faces", "orphaned_shades" ]
		.filter( schema => HBJ.json[ schema ] && HBJ.json[ schema ].length > 0 )
		.forEach( type => {
			HBJ.json[ type ].forEach( ( shade, i ) => HBJ.geometryShades.push( HBJ.addShape3d( shade, i ) ) );
		} );

	HBJ.pushMeshes( "Shade", HBJ.geometryShades, 0xffffff );

};



HBJ.pushMeshes = function ( name, geometry, color ) {

	if ( geometry.length ) {

		const bufferGeometry = THREE.BufferGeometryUtils.mergeBufferGeometries( geometry );
...
HBJ.processRoomFaces = function () {
	...
	roomFaces.forEach( ( room, i ) => room.forEach(
		face => {
			console.log( "", face );
			geometryFaces.push( HBJ.addShape3d( face, i ) );

		}
	) );

	HBJ.pushMeshes( "faces", geometryFaces, 0xff0000 );
