...
RAY.addPointerDown = function () {

	renderer.domElement.addEventListener( "pointerdown", RAY.onPointerDown );
	renderer.domElement.addEventListener( "touchstart", RAY.onPointerDown );
	renderer.domElement.addEventListener( "touchmove", RAY.onPointerDown );
...
RAY.onPointerDown = function ( event ) {
	...
	RAY.raycaster.setFromCamera( RAY.mouse, currentCamera );
	...
	if ( intersects.length ) {
		...
		if ( event.button === 0 ) {
			...
			control.attach( intersected.object );
			return;
		}
